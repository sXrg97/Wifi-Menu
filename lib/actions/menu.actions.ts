...
import { arrayRemove, collection, doc, getDoc, getDocs, orderBy, query, serverTimestamp, updateDoc, where } from "firebase/firestore";
...
export const fetchMenu = async (menuId: string) => {
  ...
  export const UpdateMenuInfo = async (
    ...
    try {
      ...
      let imageUrl: string | null = menuSnapshot.data().menuPreviewImage;
...
export const addCategory = async (menuId: string, categoryName: string) => {
    try {
      ...
      const menuData = menuSnapshot.data();
      const categoryExists = menuData.categories?.some((category: any) => category.name === categoryName);
  ...
  export const addProductToCategory = async (
    ...
    try {
      ...
      if (formData) {
        const productPicture = formData.get('productPicture');
        if (productPicture instanceof File) {
          const storageRef = ref(storage, `menus/${menuId}/products/${productPicture.name}`);
      ...
      const updatedProducts = [...(categories[categoryIndex].products || []), productWithId];
  
      // Update the entire categories array in Firestore
      const updatedCategories: { name: string; products: ProductType[] }[] = [...categories];
  ...
  export const deleteProduct = async (menuId: string, categoryName: string, productId: string) => {
    try {
        ...
        const productIndex = category.products.findIndex((prod: any) => prod._id === productId);
        ...
        const updatedProducts = [...category.products.slice(0, productIndex), ...category.products.slice(productIndex + 1)];

        // Create a new categories array with the updated products array
        const updatedCategories = [...categories];
        updatedCategories[categoryIndex] = { ...category, products: updatedProducts };
  ...
  export const deleteCategory = async (menuId: string, categoryName: string) => {
    try {
      ...
      const menuData = menuSnapshot.data();
      const categoryIndex = menuData.categories?.findIndex((cat: any) => cat.name === categoryName);
      ...
      await updateDoc(menuRef, {
        categories: [...menuData.categories.slice(0, categoryIndex), ...menuData.categories.slice(categoryIndex + 1)],
      });
  ...
  export const editProductImage = async (menuId: string, categoryName: string, productId: string, formData: FormData) => {
    ...
    try {
      ...
      // Find the category
      const category = categories.find((cat: { name: string; }) => cat.name === categoryName);
      if (!category) {
        throw new Error("Category not found");
      }
  
      // Find the product
      const product = category.products.find((prod: { _id: string; }) => prod._id === productId);
      if (!product) {
  ...
  export const editProduct = async (menuId: string, categoryName: string, productId: string, editedProduct: ProductType) => {
    try {
        ...
        // Find the category
        const categoryIndex = categories.findIndex((cat: { name: string; }) => cat.name === categoryName);
        if (categoryIndex === -1) {
            throw new Error("Category not found");
        }
        ...
        // Find the product
        const productIndex = category.products.findIndex((prod: { _id: string; }) => prod._id === productId);
        if (productIndex === -1) {
            throw new Error("Product not found");
        }
  ...
  export const callWaiter = async (menuId: string, tableNumber: number, action: boolean) => {
    try {
      ...
      // Find the table
      const table = tables.find((table: any) => table.tableNumber === tableNumber);
      if (!table) {
