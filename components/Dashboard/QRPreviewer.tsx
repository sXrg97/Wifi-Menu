...
const QRPreviewer = ({menuName, slug, tableNumber}: {menuName: string, slug: string, tableNumber?: number}) => {
  ...
  <DrawerContent>
    <div className="flex flex-col items-center justify-center max-w-[500px] p-4 mx-auto">
        {tableNumber ? <span className="text-2xl font-bold mb-4">QR pentru {menuName} - Masa {tableNumber}</span> : <span className="text-2xl font-bold mb-4">QR pentru {menuName}</span>}
        {tableNumber ? <QRCodeSVG size={256} bgColor="#FFFFFF" fgColor="#9333EA" value={`${window.location.origin}/menu/${slug}?table=${tableNumber}`} /> : <QRCodeSVG size={256} bgColor="#FFFFFF" fgColor="#9333EA" value={`${window.location.origin}/menu/${slug}`} />}
